...
function repopulateCollaboratorList(evt) {
	Util.log.console(evt);
	loadCollabaorators();
	saveToDriveFile();
...
function onFileLoaded(doc) {
  ...
  doc.addEventListener(gapi.drive.realtime.EventType.COLLABORATOR_LEFT,repopulateCollaboratorList);
  doc.addEventListener(gapi.drive.realtime.EventType.COLLABORATOR_JOINED,repopulateCollaboratorList);
  ...
  function loadCollaborators(){
	  alert(doc.getCollaborators().length);
  };
  rtclient.getFileMetadata(rtclient.params.fileIds, fileObtainedCallback);
 }

function loadCollabaorators() {
	//var strs = [ "String 1", "String 2", "String 3" ];
	var strs = doc.getCollaborators();
	var list = document.createElement("ul");
	for (var i in strs) {
	  var anchor = document.createElement("a");
	  anchor.innerText = strs[i].displayName;

	  var elem = document.createElement("li");
	  elem.appendChild(anchor);
	  list.appendChild(elem);
	  document.getElementById('collaborator_pane').appendChild(list);
	}
}

function removeCollabaorators() {
	document.getElementById('collaborator_pane').innerHTML="";
}
